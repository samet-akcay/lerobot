...
"""Pure-numpy diffusion schedulers for DDPM and DDIM inference.

These are ONNX-compatible (no PyTorch dependency at runtime) and are
parameterised entirely from the ``model.runner`` config dict in the
manifest.
"""
...
class DDPMScheduler:
    ...
    Parameters are read from the runner config dict with sensible defaults.
    """

    def __init__(self, runner_config: dict[str, Any]) -> None:
        self.num_train_timesteps: int = runner_config.get("num_train_timesteps", 1000)
        self.prediction_type: str = runner_config.get("prediction_type", "epsilon")
        self.clip_sample: bool = runner_config.get("clip_sample", True)
        self.clip_sample_range: float = runner_config.get("clip_sample_range", 1.0)

        self.betas = _compute_betas(
            self.num_train_timesteps,
            runner_config.get("beta_start", 0.0001),
            runner_config.get("beta_end", 0.02),
            runner_config.get("beta_schedule", "squaredcos_cap_v2"),
        )
...
class DDIMScheduler:
    ...
    def __init__(self, runner_config: dict[str, Any], eta: float = 0.0) -> None:
        self.num_train_timesteps: int = runner_config.get("num_train_timesteps", 1000)
        self.prediction_type: str = runner_config.get("prediction_type", "epsilon")
        self.clip_sample: bool = runner_config.get("clip_sample", True)
        self.clip_sample_range: float = runner_config.get("clip_sample_range", 1.0)
...
def create_scheduler(
    ...
    eta: float = 0.0,
) -> DDPMScheduler | DDIMScheduler | None:
    """Create a scheduler from the runner config dict.

    Returns ``None`` for the Euler scheduler (handled inline by runners).
    """
    scheduler_type = runner_config.get("scheduler", "euler").lower()
    if scheduler_type == "ddpm":
        return DDPMScheduler(runner_config)
    elif scheduler_type == "ddim":
        return DDIMScheduler(runner_config, eta=eta)
    elif scheduler_type == "euler":
        return None
    else:
        raise ValueError(f"Unknown scheduler type: {scheduler_type!r}. Supported: 'ddpm', 'ddim', 'euler'.")
