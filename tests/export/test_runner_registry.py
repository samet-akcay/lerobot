...
from lerobot.export.runners.action_chunking import ActionChunkingRunner
...
def test_select_runner_returns_expected_class_for_act() -> None:
    ...
    assert _select_runner(policy) is ActionChunkingRunner
...
def test_runtime_registry_lookup_returns_concrete_classes() -> None:
    assert next(r for r in RUNNERS if r.type == "action_chunking") is ActionChunkingRunner
...
@pytest.mark.parametrize(
    ...
    [
        (create_act_policy_and_batch, {"device": "cpu"}, ActionChunkingRunner, ["model"]),
...
def test_action_chunking_runner_export_shape() -> None:
    policy, batch = create_act_policy_and_batch()
    modules, _ = ActionChunkingRunner.export(policy, batch)
