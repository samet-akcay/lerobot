...
# Export to ONNX
policy.export("./my_policy_package")
```

> **Note**: `policy.export()` is a convenience wrapper around `lerobot.export.export_policy()`.
> You can also call it directly: `from lerobot.export import export_policy; export_policy(policy, "./my_policy_package")`.
...
runner = load_exported_policy("./my_policy_package", backend="onnx")
...
actions = runner.predict_action_chunk(observation)
...
policy.export("./diffusion_package")
```
...
runner = load_exported_policy("./my_package", backend="openvino", device="CPU")
```
